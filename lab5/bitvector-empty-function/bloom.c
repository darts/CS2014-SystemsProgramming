...
struct bloom * bloom_new(int size){
    struct bloom *theBloom = malloc(sizeof(struct bloom));
    theBloom->bit_vector = bitset_new(size);
    theBloom->size = size;
    return theBloom;
}
...
int bloom_lookup(struct bloom * this, char * item){
    ...
    if(bitset_lookup(this->bit_vector, hash1) == 0)
        return 0;
    int hash2 = hash_string(item, BLOOM_HASH2, this->size);
    if(bitset_lookup(this->bit_vector, hash2) == 0)
        return 0;
    return 1;
}
